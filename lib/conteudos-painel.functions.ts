...
async function getMeuConsultorId(
  supabase: { from: (t: "consultores") => any },
  userId: string,
): Promise<string | null> {
...
async function exigirConsultorId(
  supabase: { from: (t: "consultores") => any },
  userId: string,
): Promise<string> {
...
function toMeuConteudo(row: any): MeuConteudo {
  const area = (row.areas ?? null) as { slug: string; nome: string } | null;
  return {
    id: row.id,
    slug: row.slug,
    titulo: row.titulo,
...
export const confirmarConteudoUpload = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const patch =
      data.tipo === "poster" ? { poster_path: data.path } : { video_path: data.path };
