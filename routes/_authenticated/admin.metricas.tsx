...
function AdminMetricasPage() {
  ...
  const linhasConsultor: LeadsPorConsultor[] = [...(dados?.leadsPorConsultor ?? [])].sort((a, b) => {
        ...
        <section className="mt-6 rounded border border-border bg-card p-4">
          ...
          {invertido ? (
            <p className="mt-3 text-xs text-muted-foreground">
              A data final estava antes da inicial — o período foi invertido automaticamente ({inicio}{" "}
              a {fim}).
            </p>
          ) : null}
        </section>
                    ...
                    {linhasConsultor.map((linha) => (
                      <tr key={linha.consultorId} className="border-b border-border/60">
                        ...
                        <td className="py-2 pr-4">
                          {linha.taxa.toLocaleString("pt-BR")}%
                        </td>
                      </tr>
                    ))}
                  </tbody>
