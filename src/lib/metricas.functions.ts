...
export const exportarUsuariosCsv = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: linhas, error } = await supabase
      .from("usuarios")
      .select("nome, email, telefone, created_at, consentimento_lgpd_em, consentimento_lgpd_versao")
    ...
    const csv = montarCsv(
      ["nome", "email", "telefone", "data_cadastro", "consentimento_em", "consentimento_versao"],
      (linhas ?? []).map((u) => [
        u.nome,
        u.email,
        u.telefone,
